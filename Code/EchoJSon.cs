...
using EchoRequest.Code.Support;
...
namespace EchoRequest.Code
{
	public class EchoJSon: Page
	{
		...
		protected override void OnInit(EventArgs e)
		{
			...
			switch (service)
			{
				...
				case Service.GetTypes:
					...
					break;
				case Service.GetAppConfigServers:
					Services.AppConfig.GetAppConfigServers(Response, Request);
					break;
				case Service.GetAppConfig:
					Services.AppConfig.GetAppConfig(Response, Request);
					break;
				default:
		...
		{
			// TODO: Escape value?
			var encoded = SJavaScript.EncodeJsString(value != null ? value.ToString() : string.Empty);
			string json = string.Format("{0}\"{1}\": \"{2}\"", comma, key, encoded);
